...
a lattice $L_{N\times N \times N}, N \in \mathbb{N} $ (initially $L_{xyz}=1$) of mass and air as described before (the output of the algorithm), and a lattice $L2_{N\times N \times N}$, (initially $L2_{xyz}=-1$), of positions which belongs to any particle ($i$ if belongs to the contour or interior of the particle $i$),

where each element in $P$ has the following properties:
...
$C_{i} = \{c_{1}, ... , c_{m_{i}}\}$: (Contour) vector (set) of positions which represents the {\em contour} of the particle in $L$. The interior represents the positions which will be affected by the particle, and the contour is created to ensure avoidance with other particles.

The algorithm works as follows. Each particle evolves trying to extend its occupied positions ($O$) by marking positions in $C$. When a position is marked, it is added to its $O$ vector and the neighbourhood of that position {\em i.e.}, the lattice positions that surround it, are added to the contour vector $C$. Also the lattices are updated: $L$ is set to $0$ in the position (the air bubble now is bigger), and $i$ is set in $L2$ to the neighbourhood positions. In practice, the neighbourhood size is a parameter, so different distances between particles could be defined.
...
Random positions are chosen in the examples, and then the system is solved at each time step using a runge-kutta solver of fourth order\citep{aa} algorithm to determine the new position of the trajectory. For instance, in the left image, the trajectories are {\em attracted} by a position which lies in the left-upper quadrant. Trajectories could also be attracted to a set of point instead to a particular one. In the right image, most of the trajectories follow a circle without having a specific attractor.
